...
3. Papers in higher-impact journals (as measured by impact factor) will be more likely to have code and data. Covariate: `Impact_Factor`
...
mod1 <- rstanarm::stan_glmer(
          Data_and_Code ~ Code_Required + scale(Year) + scale(Impact_Factor) + (1|Journal),
...
2. The more outside libraries the code depends on, the less likely the code will be to run. This includes all dependencies of libraries called in the script. Covariate: `Libraries`
...
mod2 <- rstanarm::stan_glmer(
          All_Code_Runs ~ scale(Code_Lines) + scale(Libraries) + Code_Format + (1|Journal),
...
There was a positive effect of year on the probability both code and data were available, though the 95% credible interval slightly overlapped 0 (Table 1, Figure 2).
The probability a paper had both data and code accessible did not differ based on code sharing policy or journal impact factor (Table 1, Figure 2).
...
code_pr <- apply(code_post, 2, mean)
```
...
The probability that a paper's code ran successfully was not related to the number of libraries used or code format (Table 2,  Figure 3).
...
Estimates from from the generalized linear mixed model of probability a paper provided both code and data, as a function of journal code policy, year, and journal impact factor.
...
tab1 <- tab1[1:5,]
tab1$Parameter <- c("Intercept","Code required", "Year", "Impact factor", "Journal SD")
...
Estimates from from the generalized linear mixed model of probability a paper's code ran successfully, as a function of number of lines of code, number of R libraries required, and code format (relative to a reference level of R script).
Intercepts were random by journal.
...
tab2 <- tab2[1:6,]
tab2$Parameter <- c("Intercept","Code lines", "Libraries", "Rmd/other", "PDF/Word", "Journal SD")
