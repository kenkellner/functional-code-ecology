...
year_post <- as.matrix(mod1)[,"scale(Year)"]
...
There was a positive effect of year on the probability both code and data were available, with `r pct_pos`% of the posterior samples for the parameter greater than 0. However, the 95% credible interval slightly overlapped 0 (Table 1, Figure 2).
...
The probability that a paper's code ran successfully was not related to the number of packages used or code format (Table 2,  Figure 3).
...
Effects of code lines (A), number of required R packages (B), and code format (C) on proportion of papers for which provided code ran successfully.
